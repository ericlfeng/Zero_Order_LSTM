...
##############################################################################
# Run N-layer LSTM overfitting sweeps with Backpropagation (BPTT)
# Simplified version - no perturbation-based hyperparameters needed
##############################################################################
...
########## RUN PREFIX #############
RUN_PREFIX="lstm${n_layers}L_overfit_bptt"
...
WANDB=true
WANDB_PROJ="Zero_Order_Opt_LSTM_Overfit_BPTT"
                                        ...
                                        for GRAD_CLIP in "${GRAD_CLIPS[@]}"; do
                                            for OVERFIT in "${OVERFITS[@]}"; do
                                              ...
                                              python rge_series_experiments.py \
                                                    ...
                                                    --warmup_iters 1 \
                                                    --seed 42 \
                                                    --output_dir ./results_lstm${n_layers}layer_overfit_bptt \
                                                    ${EXTRA_FLAGS} \
                                                    ;
...
echo "[INFO] Results directory: ./results_lstm${n_layers}layer_overfit_bptt"